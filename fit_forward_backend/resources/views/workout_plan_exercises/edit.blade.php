...
<div class="container col-md-10 col-lg-8 mt-5">
    <div class="card shadow">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Edit Workout Plan Exercise</h5>
            <a href="{{ route('workout_plan_exercises.index', $wid) }}" 
                class="btn btn-dark btn-sm">Back to List</a>
        </div>

        <div class="card-body">
            <form action="{{ route('workout_plan_exercises.update', 
                ['wid' => $wid, 'id' => $workout_plan_exercises->id]) }}" 
                method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="exercise_id" id="exercise_id" class="form-select" required>
                        ...
                        @foreach ($exercises as $category)
                            <option value="{{ $category->id }}" {{ $workout_plan_exercises->exercise_id == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" min="1" name="sets" id="sets" class="form-control" 
                            value="{{ $workout_plan_exercises->sets }}" required>
                        @error('sets')
                            <span class="text-danger text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" min="1" name="reps" id="reps" class="form-control" 
                            value="{{ $workout_plan_exercises->reps }}">
                        @error('reps')
                            <span class="text-danger text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" min="1" name="duration" id="duration" class="form-control" 
                            value="{{ $workout_plan_exercises->duration }}">
                        @error('duration')
                            <span class="text-danger text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary">Edit Workout Plan Exercise</button>
                </div>
            </form>
        </div>
    </div>
