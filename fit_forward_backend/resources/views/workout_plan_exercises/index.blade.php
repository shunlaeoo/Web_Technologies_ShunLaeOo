...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold">Workout Plan Exercises</h4>
        <a href="{{ route('workout_plan_exercises.create', $id) }}" class="btn btn-primary">
            ...
            <tbody>
                @foreach($workout_plan_exercises as $index => $category)
                    <tr>
                        ...
                        <td>{{ $category->exercise->name }}</td>
                        <td>{{ $category->sets }}</td>
                        <td>{{ $category->reps }}</td>
                        <td>{{ $category->duration }}</td>
                        <td>
                            <a href="{{ route('workout_plan_exercises.edit', ['wid' => $id, 'id' => $category->id]) }}" 
                            ...
                            <form action="{{ route('workout_plan_exercises.delete', ['wid' => $id, 'id' => $category->id]) }}" 
                                method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger" title="Delete"
                                    onclick="return confirm('Are you sure you want to delete this workout plan exercise?')">
                                    <svg class="pb-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" 
                                        width="18" fill="currentColor" viewBox="0 0 24 24">
